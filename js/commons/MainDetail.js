    ...
    if (this.props.bookDetail) {
      ...
      if (this.props.bookmarks) {
        ...
        let index = _findIndex(this.props.bookmarks, item => {
          return (
            item.bookName === this.props.bookDetail.title &&
            item.author === this.props.bookDetail.author
          );
        });
            ...
            {this.props.isFetchingDetail ? (
              <View
                ...
                <ActivityIndicator
                  ...
                  size="small"
                />
                <Text style={{color: appTheme.primaryColor}}>
                  加载中，请稍等
                </Text>
              </View>
            ) : null}
  ...
  _renderInfo = color => {
    return (
      <View style={styles.bookInfo}>
        <FastImage
          source={{uri: ZSSQ_IMG_URL + this.props.bookDetail.cover}}
          style={styles.bookImage}
        />
        <View style={{justifyContent: 'space-around', marginLeft: 12}}>
          <Text style={styles.bookTitle}>
            {getChineseText(this.props.bookDetail.title)}
          </Text>
          <View style={styles.bookAuthor}>
            <Text style={{color: color}}>
              {getChineseText(this.props.bookDetail.majorCate)}
            </Text>

            <Text> | {getChineseText(this.props.bookDetail.author)}</Text>
          </View>
          <Text style={{color: '#292929'}}>
            {getChineseText(this.props.bookDetail.latelyFollower)}人在追
          </Text>
          <Text style={{color: '#292929'}}>
            字数：{getChineseText(this.props.bookDetail.wordCount)}
          </Text>

          <Text style={{color: '#292929'}}>
            状态：
            {this.props.bookDetail.isSerial ? I18n.t('serial') : I18n.t('over')}
          </Text>
        </View>
      </View>
    );
        ...
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          <Text
            style={{
              height: 18,
              width: 4,
              backgroundColor: this.props.appTheme.primaryColor,
              marginRight: 4,
            }}
          />
          <Text style={{marginVertical: 15, fontSize: 18, fontWeight: 'bold'}}>
            详情
          </Text>
        </View>
      ...
      <TouchableOpacity
        ...
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          ...
          <Text numberOfLines={1}>
            {' '}
            {getChineseText(this.props.bookDetail.lastChapter)}
          </Text>
        </View>
    ...
    return (
      <TouchableOpacity
        ...
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          ...
          <Text numberOfLines={1}>
            共{this.props.bookDetail.chaptersCount}章
          </Text>
        </View>
