...
class GuessYouLike extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (nextProps.guessYouLikeData) {
      try {
        this.data = _chunk(nextProps.guessYouLikeData.ranking.books, 4);
    ...
    return (
      this.props.guessYouLikeData !== nextProps.guessYouLikeData ||
  ...
  render() {
    return this.books && this.books.length > 0 ? (
      <View style={styles.container}>
        <CardHeader
          tagColor={this.props.appTheme.primaryColor}
          title={I18n.t('guessYouLike')}
        />
