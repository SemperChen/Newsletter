...
class MainSearchContent extends React.Component {
  ...
  render() {
    ...
    return (
      <View>
        {this.bookInfo &&
        this.props.searchBookName &&
        this.bookInfo.title === this.props.searchBookName ? (
    ...
    return (
      <TouchableHighlight
        ...
        <View style={styles.bookInfo}>
          <FastImage
            source={{uri: ZSSQ_IMG_URL + this.bookInfo.cover}}
            style={styles.bookImage}
          />
          <View style={{justifyContent: 'space-around', marginLeft: 12}}>
            <Text style={styles.bookTitle}>
              {getChineseText(this.bookInfo.title)}
            </Text>
  ...
  _renderChapterCatalog = () => {
    return (
      <View style={[styles.margin, styles.chapterCatalog]}>
        <TouchableHighlight>
          <Text>
            {I18n.t('newChapter')} {getChineseText(this.bookInfo.lastChapter)}
          </Text>
        </TouchableHighlight>
        <TouchableHighlight>
