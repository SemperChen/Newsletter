...
class SaveBookContent extends React.Component {
    ...
    cacheArticle = (content) => {
        try {
            ...
            let fileName = this.props.siteName.replace('_','') +'pageNum'+ this.startNum +'.txt';
            ...
            if(globalData.cacheChapterCount%50===0){
                saveCatalog(globalData.catalog, this.props.articleTag, globalData.cacheChapterCount)
            }
            //写入缓存
    ...
    fetchArticlesCache = () => {
        ...
        switch (this.siteName) {
            case ZSSQ_NAME:
                try {
                    url = ZSSQ_CHARTER_BASE + globalData.catalog[this.startNum].link
   ...
   /* hint = () => {
    ...
    async shouldComponentUpdate(nextProps, nextState) {
        try {
            if ((nextProps.article && this.props.article !== nextProps.article && this.startNum < this.endNum) ||
                (nextProps.article && nextProps.article === REQUEST_CACHE_FAILED && this.startNum < this.endNum)) {
                let content = '';
                try {
                    ...
                    content = '';
                    console.warn('SaveBookContent shouldComponentUpdate func err', e.message)
                }
                ...
                for (;this.startNum < this.endNum;) {
                    ...
                    if(globalData.catalog&&(!globalData.catalog[this.startNum].isCacheSuccess)) break
                }
            ...
            return this.state.isShowCacheBook !== nextState.isShowCacheBook ||
                ...
                <View>
                    ...
                    >
                        <View style={styles.container}
                            ...
                            <View style={[styles.content,{backgroundColor:NovelAppConfig.readerColor.contentBgColor}]}>

                                <View style={styles.modalHeader}>
                                    <Text
                                        style={[styles.btnText,{color:NovelAppConfig.readerColor.sliderColor,fontWeight: 'bold'}]}
                                        >共{this.props.catalogLength}章，
                                        已{getChineseText('缓存') + globalData.cacheChapterCount}章
                                    </Text>
                                ...
                                >
                                    <Text style={[styles.btnText,{color:NovelAppConfig.readerColor.sliderColor}]}>{getChineseText('缓存后面一百五十章')}</Text>
                                </TouchableOpacity>
                                ...
                                >
                                    <Text style={[styles.btnText,{color:NovelAppConfig.readerColor.sliderColor}]}>{getChineseText('缓存后面全部')}</Text>
                                </TouchableOpacity>
                                <TouchableOpacity
                                    style={styles.btn}
                                    onPress={() => {
                                ...
                                >
                                    <Text style={[styles.btnText,{color:NovelAppConfig.readerColor.sliderColor}]}>{getChineseText('缓存全部')}</Text>
                                </TouchableOpacity>
                                <TouchableOpacity
                                    style={styles.btn}
                                    onPress={() => {
                                ...
                                >
                                    <Text style={[styles.btnText,{color:NovelAppConfig.readerColor.sliderColor}]}>取消</Text>
                                </TouchableOpacity>
                            </View>
...
export default connect(mapStateToProps, null, null, {withRef: true})(SaveBookContent)
