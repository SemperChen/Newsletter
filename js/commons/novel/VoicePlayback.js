...
class VoicePlayback extends React.Component{
    ...
    shouldComponentUpdate(nextProps,nextState){
        return this.props.text!==nextProps.text||this.state.text!==nextState.text
    }
    ...
    setVoiceText = (text, isTurn2NextPage) => {
        ...
        if(text.substring(text.length-2)!=='。'
            ...
            ||text.substring(text.length-2)!=='?'){
            if(isTurn2NextPage){
                t = this.overflowText + text.substring(0,punctuationMark.markIndex+1);
            }else {
    ...
    render(){
        ...
        return(
            <Video source={{uri: encodeURI('http://tsn.baidu.com/text2audio' +
                    '?lan=zh&ctp=1&cuid=abcdxxx' +
                    '&tok=' + NovelAppConfig.baiduVoiceTok +
                    '&per=3' +
                    '&spd=5' +
                    '&pit=5' +
                    '&vol=9' +
