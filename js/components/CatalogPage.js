...
class CatalogPage extends React.Component {
    ...
    render() {
        ...
        return (
            <View
                style={[styles.catalogContainer, {backgroundColor: NovelAppConfig.readerColor.contentBgColor}]}
                ...
                {globalData.catalog
                    ...
                    >
                        <View style={{
                            ...
                            <Text style={{color:NovelAppConfig.readerColor.sliderColor,fontSize:20,marginBottom: 10}}>{this.bookName}</Text>

                            <View style={{
                                flexDirection: 'row',
                                justifyContent: 'space-between',
                            }}>
                                <Text style={{color:NovelAppConfig.readerColor.sliderColor}}>共{globalData.catalog.length}章</Text>
                                <View style={{flexDirection:'row'}}>
                                    ...
                                    >
                                        <MaterialIcons name="keyboard-arrow-down" size={25} style={{marginRight:10,color:NovelAppConfig.readerColor.sliderColor}}/>

                                    </TouchableOpacity>
                                    <TouchableOpacity
                                        onPress={()=>{
                                            this._flatRef.scrollToOffset({offset:0,animated:false})
                                        }}
                                    >
                                        <MaterialIcons name="keyboard-arrow-up" size={25} style={{color:NovelAppConfig.readerColor.sliderColor}}/>
    ...
    _onViewableItemsChanged = () => {
        ...
        if (this.currentChapterNum && globalData.catalog.length > 0 && !this.isFetched) {
            this.isFetched = true;
            try {
