...
import {
  getBookContentEXIAOSHUO1,
  getCatalog1,
  getCatalogEXIAOSHUO1,
  getContent1,
...
class ReadPage extends React.Component {
  ...
  setIsShowAd = () => {
    ...
    if (date - showAdInterval > NovelAppConfig.lastShowAdTime) {
      this.isShowAd =
        ...
        NovelAppConfig.VIPExperienceDays < 1 &&
        NovelAppConfig.isShowBannerAd;
  ...
  setCurrentArticle = article => {
    try {
      ...
      if (NovelAppConfig.isOpenVoice) {
        this.player.setVoiceText(
          this.currentArticle.chapterContent,
          this.isTurn2NextPage,
        );
      }
    } catch (e) {
  ...
  changeBookSource = (articleUrlTag, title, img, siteName, bookId, author) => {
    ...
    this.lastChapterNum = this.currentArticle
      ? this.currentArticle.currentChapterNum - 1
  ...
  shouldComponentUpdate(nextProps) {
    ...
    if (
      !this.isFirstGetCatalog &&
      nextProps.catalogData &&
      !this.isChangeBookSource
    ) {
      ...
      if (globalData.catalog && globalData.catalog.length > 0) {
        globalData.catalog = globalData.catalog.concat(
          _drop(catalogData, globalData.catalog.length),
        );
      } else {
  ...
  fetchCatalogData = () => {
    try {
      switch (this.siteName) {
        case ZSSQ_NAME:
          this.props.dispatch(
            requestCatalog(ZSSQ_CATALOG_BASE + this.bookId, RECEIVE_TYPE.JSON),
          );
          break;
        case EXIAOSHUO5_NAME:
          this.props.dispatch(
            requestCatalog(this.articleTag, RECEIVE_TYPE.TEXT),
          );
          break;
        default:
          this.props.dispatch(
            requestCatalog(this.articleTag, RECEIVE_TYPE.GBKTEXT),
          );
      }
    } catch (e) {
  ...
  fetchArticles = () => {
    ...
    if (
      ...
      this.props.catalogData !== REQUEST_CATALOG_FAILED
    ) {
      let chapterCount = 0;
      switch (this.siteName) {
        case ZSSQ_NAME:
    ...
    switch (this.siteName) {
      case ZSSQ_NAME:
        try {
          url =
            ZSSQ_CHARTER_BASE +
            globalData.catalog[this.lastChapterNum - 1].link
              .replace('/', '%2F')
              .replace('?', '%3F');
      ...
      default:
        try {
          url =
            this.articleTag + globalData.catalog[this.lastChapterNum - 1].link;
          this.lastRequestUrl = url;
        } catch (e) {
          this.props.dispatch(receiveArticle(REQUEST_ARTICLE_FAILED, url));
    ...
    try {
      ...
      if (globalData.catalog && globalData.catalog.length > 0) {
        ...
        if (articleCache && articleCache.isCacheSuccess) {
          ...
          RNFS.readFile(articleCache.filePath, 'utf8')
            .then(res => {
              // console.log('res',res)
  ...
  fetchArticlesByCache = (url, cacheArticle) => {
    switch (this.siteName) {
      case ZSSQ_NAME:
        this.props.dispatch(
          requestArticle(url, RECEIVE_TYPE.JSON, cacheArticle),
        );
        // this.props.dispatch(requestBookCache(url, RECEIVE_TYPE.JSON));
        break;
      case EXIAOSHUO5_NAME:
        this.props.dispatch(
          requestArticle(url, RECEIVE_TYPE.TEXT, cacheArticle),
        );
        break;
      default:
        this.props.dispatch(
          requestArticle(url, RECEIVE_TYPE.GBKTEXT, cacheArticle),
        );
        break;
    }
  };
  ...
  _openBookCatalog = () => {
    try {
      this.props.navigation.navigate('Catalog', {
        ...
        currentChapterNum: this.currentArticle
          ? this.currentArticle.currentChapterNum + 1
          : this.lastChapterNum + 1,
  ...
  _navToSearch = () => {
    ...
    this.props.navigation.navigate('BookSources', {
      changeBookSource: this.changeBookSource,
    });
  };
  ...
  _showIsSaveBookcase = () => {
    if (!this.bookmark && this.currentArticle) {
      Alert.alert(I18n.t('reminder'), I18n.t('isSaveBook'), [
        {
          text: I18n.t('cancel'),
          style: 'cancel',
          onPress: () => {
  ...
  _toggleShowCacheBook = () => {
    try {
      if (this.currentArticle) {
        this.saveBookContent
          .getWrappedInstance()
          .setLastChapterNum(this.currentArticle.currentChapterNum);
      }
  ...
  tintReader = () => {
    Alert.alert(I18n.t('reminder'), I18n.t('offlineTint'), [
      {
        text: I18n.t('confirm'),
        onPress: () => {},
      },
  ...
  render() {
    ...
    //请求章节内容失败，渲染以下UI
    if (
      this.props.article === REQUEST_ARTICLE_FAILED &&
    ...
    if (
      ...
      switch (this.siteName) {
        default:
          ...
          articles = getBookContentEXIAOSHUO1(
            this.props.article,
